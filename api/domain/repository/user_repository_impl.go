...
func (userRepositoryImpl *UserRepositoryImpl) FindAll() ([]*domain.User, error) {
...
func (userRepositoryImpl *UserRepositoryImpl) FindById(id int) (*domain.User, error) {
...
func (userRepositoryImpl *UserRepositoryImpl) DeleteById(id int) error {
	...
	userRepositoryImpl.db.Transaction(func(tx *gorm.DB) error {
		err := tx.Delete(&domain.User{}, id).Error
		if err != nil {
			return err
		}

		err = tx.Where("user_id = ?", id).Delete(&domain.Card{}).Error
		if err != nil {
			return err
		}
...
func (userRepositoryImpl *UserRepositoryImpl) Save(user *domain.User) (*domain.User, error) {
	var userCheck = new(domain.User)
...
func (userRepositoryImpl *UserRepositoryImpl) Update(user *domain.User) (*domain.User, error) {
